...
class Context:
    def __init__(self, user_id: str, client_id: str, options: Dict[str, Any], client: httpx.AsyncClient):
...
async def build_api(global_options: Dict[str, Any], html: str, client: httpx.AsyncClient):
    fb_dtsg = None
    iris_seq_id = None
    ...
    if not user_id or user_id == "0":
        ...
        user_id_match = re.search(r'\["CurrentUserInitialData",\[\],{"USER_ID":"(\d+)"}', html)
        if user_id_match and user_id_match.group(1) != "0":
            user_id = user_id_match.group(1)
...
async def handle_redirect(res: httpx.Response, client: httpx.AsyncClient, global_options: Dict[str, Any]) -> httpx.Response:
    if res.status_code in (301, 302, 303, 307, 308) and "Location" in res.headers:
        ...
        res = await client.get(redirect_url, headers=get_headers(redirect_url, global_options))
...
async def make_login(client: httpx.AsyncClient, email: str, password: str, global_options: Dict[str, Any]) -> httpx.Response:
    ...
    res = await client.get("https://www.facebook.com/", headers=get_headers("https://www.facebook.com/", global_options))
    ...
    res = await client.post(login_url, data=form, headers=get_headers(login_url, global_options))
    return await handle_redirect(res, client, global_options)

async def login(app_state: Optional[List[Dict[str, Any]]] = None, email: Optional[str] = None, password: Optional[str] = None, options: Optional[Dict[str, Any]] = None):
    global_options = {
        "selfListen": False,
        "listenEvents": True,
        "listenTyping": False,
    ...
    try:
        ...
        if not res or not res.text:
            res = await client.get("https://www.facebook.com/messages", headers=get_headers("https://www.facebook.com/messages", global_options))
            res = await handle_redirect(res, client, global_options)
