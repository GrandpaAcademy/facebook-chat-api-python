...
def format_messages_graphql_response(data: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    if not message_thread:
        return []
    thread_id = message_thread["thread_key"].get("thread_fbid") or message_thread[
        "thread_key"
    ].get("other_user_id")
    ...
    for d in message_thread.get("messages", {}).get("nodes", []):
        ...
        if typename == "UserMessage":
            ...
            messages.append(
                {
                    ...
                    "attachments": (
                        sticker_attachment
                        if sticker_attachment
                        else (
                            [
                                format_attachments_graphql_response(a)
                                for a in d.get("blob_attachments", [])
                            ]
                            + (
                                [
                                    format_extensible_attachment(
                                ...
                                if d.get("extensible_attachment")
                                else []
                            )
                        )
                    ),
                    "body": (
                        d.get("message", {}).get("text") if d.get("message") else ""
                    ),
                    ...
                    "messageReactions": (
                        [
                            format_reactions_graphql(r)
                            for r in d.get("message_reactions", [])
                        ]
                        if d.get("message_reactions")
                        else None
                    ),
            ...
            event_type = (
                event_type_map.get(typename)
                or d.get("extensible_message_admin_text_type", "").lower()
            )
            ...
            if typename == "ThreadNameMessage":
                ...
                event_data = {
                    "participantsRemoved": [
                        p["id"] for p in d.get("participants_removed", [])
                    ]
                }
            elif typename == "ParticipantsAddedMessage":
                event_data = {
                    "participantsAdded": [
                        p["id"] for p in d.get("participants_added", [])
                    ]
                }
...
async def get_thread_history(
    post_func, ctx: Any, thread_id: str, amount: int, timestamp: Optional[int] = None
):
    form = {
        "av": ctx.user_id,
        "queries": json.dumps(
