...
def format_attachments_graphql_response(attachment: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if typename == "MessageImage":
        return {
            ...
            "original_extension": get_extension(
                attachment.get("original_extension", ""), attachment.get("filename", "")
            ),
            ...
            "attributionApp": (
                ...
                if attachment.get("attribution_app")
                else None
            ),
        }
...
def format_extensible_attachment(attachment: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if story:
        ...
        return {
            ...
            "description": (
                story.get("description", {}).get("text")
                if story.get("description")
                else None
            ),
            "source": (
                story.get("source", {}).get("text") if story.get("source") else None
            ),
            "image": (
                (media.get("animated_image") or media.get("image", {})).get("uri")
                if media and (media.get("animated_image") or media.get("image"))
                else None
            ),
            "width": (
                (media.get("animated_image") or media.get("image", {})).get("width")
                if media and (media.get("animated_image") or media.get("image"))
                else None
            ),
            "height": (
                (media.get("animated_image") or media.get("image", {})).get("height")
                if media and (media.get("animated_image") or media.get("image"))
                else None
            ),
            ...
            "properties": {
                p["key"]: p["value"].get("text") for p in story.get("properties", [])
            },
        }
...
def format_event_data(event: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if typename == "ThemeColorExtensibleMessageAdminText":
        ...
        return {
            "accepter_id": event.get("accepter_id"),
            "requester_id": event.get("requester_id"),
        }
    elif typename == "ConfirmFriendRequestExtensibleMessageAdminText":
        return {
            "friend_request_recipient": event.get("friend_request_recipient"),
            "friend_request_sender": event.get("friend_request_sender"),
        }
    elif typename == "AddContactExtensibleMessageAdminText":
        return {
            "contact_added_id": event.get("contact_added_id"),
            "contact_adder_id": event.get("contact_adder_id"),
        }
