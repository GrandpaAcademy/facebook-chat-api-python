...
def format_thread_graphql_response(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    return {
        ...
        "name": t.get("name"),
        "participantIDs": [
            e["node"]["messaging_actor"]["id"] for e in t["all_participants"]["edges"]
        ],
        ...
        "emoji": (
            t.get("customization_info", {}).get("emoji")
            if t.get("customization_info")
            else None
        ),
        "color": (
            t.get("customization_info", {}).get("outgoing_bubble_color", "")[2:]
            if t.get("customization_info")
            and t["customization_info"].get("outgoing_bubble_color")
            else None
        ),
        "nicknames": (
            {
                v["participant_id"]: v["nickname"]
                for v in t.get("customization_info", {}).get(
