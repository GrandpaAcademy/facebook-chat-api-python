...
async def suggest_friend(
    post_func, ctx: Any, count: int = 30, cursor: Optional[str] = None
):
    form = {
        "av": ctx.user_id,
        "__aaid": 0,
    ...
    if (
        ...
        and "viewer" in res_data["data"]
        and res_data["data"]["viewer"].get("people_you_may_know")
    ):
            ...
            suggestions.append(
                {
                    ...
                    "profilePicture": (
                        node.get("profile_picture", {}).get("uri")
                        if node.get("profile_picture")
                        else None
                    ),
                    "mutualFriends": node.get("social_context", {}).get("text", ""),
                    "topMutualFriends": node.get(
