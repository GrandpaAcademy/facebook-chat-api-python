...
from ..http.remove_user_from_group import remove_user_from_group as http_remove_user_from_group
from ..http.change_admin_status import change_admin_status as http_change_admin_status
from ..http.send_typing_indicator import send_typing_indicator as http_send_typing_indicator
...
from ..graphql.set_message_reaction import set_message_reaction as gql_set_message_reaction
...
from ..http.change_archived_status import change_archived_status as http_change_archived_status
...
from ..http.handle_friend_request import handle_friend_request as http_handle_friend_request
...
from ..http.handle_message_request import handle_message_request as http_handle_message_request
from ..http.change_cover import change_cover as http_change_cover

def get_api(ctx: Any):
    return {
        "sendMessage": lambda msg, thread_id, is_single_user=None, reply_to_message=None: http_send_message(post, ctx, msg, thread_id, is_single_user, reply_to_message),
        ...
        "getThreadList": lambda limit, timestamp=None, tags=[""]: gql_get_thread_list(post, ctx, limit, timestamp, tags),
        "markAsRead": lambda thread_id, read=True: http_mark_as_read(post, ctx, thread_id, read),
        "setTitle": lambda new_title, thread_id: http_set_title(post, ctx, new_title, thread_id),
        "logout": lambda: http_logout(post, ctx),
        "setActiveStatus": lambda is_active: gql_set_active_status(post, ctx, is_active),
        "getThreadHistory": lambda thread_id, amount, timestamp=None: gql_get_thread_history(post, ctx, thread_id, amount, timestamp),
        "deleteMessage": lambda message_ids: http_delete_message(post, ctx, message_ids),
        "unsendMessage": lambda message_id: http_unsend_message(post, ctx, message_id),
        "editMessage": lambda text, message_id: ctx.mqtt_client.edit_message(text, message_id) if hasattr(ctx, "mqtt_client") else None,
        "changeNickname": lambda nickname, thread_id, participant_id: http_change_nickname(post, ctx, nickname, thread_id, participant_id),
        "changeThreadColor": lambda color, thread_id: gql_change_thread_color(post, ctx, color, thread_id),
        "changeThreadEmoji": lambda emoji, thread_id: http_change_thread_emoji(post, ctx, emoji, thread_id),
        "addUserToGroup": lambda user_id, thread_id: http_add_user_to_group(post, ctx, user_id, thread_id),
        "removeUserFromGroup": lambda user_id, thread_id: http_remove_user_from_group(post, ctx, user_id, thread_id),
        "changeAdminStatus": lambda thread_id, admin_ids, admin_status: http_change_admin_status(post, ctx, thread_id, admin_ids, admin_status),
        "sendTypingIndicator": lambda thread_id, is_typing, is_group=None: http_send_typing_indicator(post, lambda t_id: http_get_user_info(get, ctx, t_id), ctx, thread_id, is_typing, is_group),
        ...
        "markAsDelivered": lambda thread_id, message_id: http_mark_as_delivered(post, ctx, thread_id, message_id),
        "getFriendsList": lambda: http_get_friends_list(post, ctx),
        "setMessageReaction": lambda reaction, message_id: gql_set_message_reaction(post, ctx, reaction, message_id),
        "uploadAttachment": lambda attachments: http_upload_attachment(post, ctx, attachments),
        "changeAvatar": lambda image, caption="", timestamp=None: http_change_avatar(post, ctx, image, caption, timestamp),
        
        # Thread Management
        "deleteThread": lambda thread_ids: http_delete_thread(post, ctx, thread_ids),
        "muteThread": lambda thread_id, duration: http_mute_thread(post, ctx, thread_id, duration),
        "changeArchivedStatus": lambda thread_ids, archive: http_change_archived_status(post, ctx, thread_ids, archive),
        "searchForThread": lambda name: http_search_for_thread(post, ctx, name),
        "getThreadPictures": lambda thread_id, offset, limit: http_get_thread_pictures(post, ctx, thread_id, offset, limit),
        "getThreadTheme": lambda thread_id: gql_get_thread_theme(post, ctx, thread_id),
        "setThreadTheme": lambda thread_id, theme_id: gql_set_thread_theme(ctx, thread_id, theme_id),

        # User & Profile
        "changeBio": lambda bio, publish=False: http_change_bio(post, ctx, bio, publish),
        "changeName": lambda first, middle, last: http_change_name(post, ctx, first, middle, last),
        ...
        "getAvatarUser": lambda user_ids, size=50: http_get_avatar_user(get, ctx, user_ids, size),

        # Social & Interaction
        "sendFriendRequest": lambda user_id: http_send_friend_request(post, ctx, user_id),
        "handleFriendRequest": lambda user_id, accept: http_handle_friend_request(post, ctx, user_id, accept),
        ...
        "handleMessageRequest": lambda thread_id, accept: http_handle_message_request(post, ctx, thread_id, accept),
        "searchFriends": lambda query: gql_search_friends(post, ctx, query),
        "suggestFriend": lambda count=30, cursor=None: gql_suggest_friend(post, ctx, count, cursor),
        ...
        "shareContact": lambda text, sender_id, thread_id: http_share_contact(ctx, text, sender_id, thread_id),
        "shareLink": lambda text, url, thread_id: http_share_link(ctx, text, url, thread_id),
        "forwardAttachment": lambda attachment_id, user_or_users: http_forward_attachment(post, ctx, attachment_id, user_or_users),
        "createPoll": lambda title, thread_id, options=None: http_create_poll(post, ctx, title, thread_id, options),

        # Post & Stories
        "createPost": lambda msg_info: http_create_post(post, ctx, msg_info),
        "createCommentPost": lambda msg_info, post_id, reply_id=None: http_create_comment_post(post, ctx, msg_info, post_id, reply_id),
        "setPostReaction": lambda post_id, type: http_set_post_reaction(post, ctx, post_id, type),
        "setStoryReaction": lambda story_id, react: http_set_story_reaction(post, ctx, story_id, react),
