...
async def create_poll(post_func, ctx: Any, title: str, thread_id: str, options: Optional[Dict[str, bool]] = None):
    if not options:
        options = {}
    ...
    if res_data and "payload" in res_data and res_data["payload"].get("status") != "success":
