...
async def check_note(post_func, ctx: Any):
    ...
    current_note = res_data.get("data", {}).get("viewer", {}).get("actor", {}).get("msgr_user_rich_status")
    ...
    return {
        ...
        "expiresAt": (current_note["created_time"] * 1000) + (24 * 60 * 60 * 1000) if current_note else None
    }

async def create_note(post_func, ctx: Any, text: str, options: Optional[Dict[str, Any]] = None):
    if not options:
        options = {}
