...
async def change_admin_status(
    ...
    ctx: Any,
    thread_id: str,
    admin_ids: Union[str, List[str]],
    admin_status: bool,
):
    if isinstance(admin_ids, str):
        admin_ids = [admin_ids]

    form = {"thread_fbid": thread_id, "add": "true" if admin_status else "false"}

    for i, u_id in enumerate(admin_ids):
        form[f"admin_ids[{i}]"] = u_id

    res = await post_func(
        "https://www.facebook.com/messaging/save_admins/?dpr=1", ctx, form
    )
    ...
    if res_data.get("error"):
        ...
        if error_code == 1976004:
            ...
            raise Exception(
                f"Cannot alter admin status: unknown error code {error_code}."
            )
