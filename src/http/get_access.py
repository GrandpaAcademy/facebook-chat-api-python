...
async def get_access(ctx: Any, auth_code: str = ""):
    ...
    async def submit_code(code: str):
        ...
        post_res = await ctx.client.post(base_url + "security/twofactor/reauth/enter/", data=form, headers={**ctx.client.headers, **post_headers})
        ...
        try:
            ...
            if res_json.get("payload") and not res_json["payload"].get("codeConfirmed"):
                 raise Exception(res_json["payload"].get("message", "Code confirmation failed"))
        except:
    ...
    if auth_code and len(auth_code) == 6:
        ...
        if token_match:
            ...
            raise Exception("2FA required or Access token not found. Please provide an auth_code.")
