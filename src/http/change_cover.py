...
async def change_cover(post_func, ctx: Any, image: Any):
    ...
    if not res_upload_data or (res_upload_data.get("error") or res_upload_data.get("errors") or not res_upload_data.get("payload")):
    ...
    try:
        return res_gql_data["data"]["user_update_cover_photo"]["user"]["cover_photo"]["photo"]["url"]
    except:
        return res_gql_data
