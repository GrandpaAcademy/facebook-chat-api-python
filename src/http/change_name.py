...
async def change_name(post_func, ctx: Any, name_info: dict, format: str = "complete"):
    ...
    full_name = ""
    if format == "complete":
        full_name = f"{last_name} {middle_name + ' ' if middle_name else ''}{first_name}"
    ...
    if res_data and "data" in res_data and res_data["data"].get("fxim_update_identity_name", {}).get("error"):
        raise Exception(f"changeName data error: {res_data['data']['fxim_update_identity_name']['error']}")
