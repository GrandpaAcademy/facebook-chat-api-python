...
async def change_avatar(post_func, ctx: Any, image: Any, caption: str = "", timestamp: Optional[int] = None):
    ...
    res = await ctx.client.post(upload_url, data=form, files=files, headers=ctx.client.headers)
    res_data = parse_and_check_login(ctx, res)
    if not res_data or res_data.get("error"):
        raise Exception(f"changeAvatar upload error: {res_data.get('error') if res_data else 'Empty response'}")
    ...
    if not res_gql_data or res_gql_data.get("errors"):
        raise Exception(f"changeAvatar mutation error: {res_gql_data.get('errors') if res_gql_data else 'Empty response'}")
