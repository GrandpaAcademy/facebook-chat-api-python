...
async def create_post(post_func, ctx: Any, msg_info: Union[str, Dict[str, Any]]):
    ...
    variables = {
        "input": {
            "composer_entry_point": "share_modal" if (not group_id and url_attachment) else "inline_composer",
            "composer_source_surface": "feed_story" if (not group_id and url_attachment) else "group" if group_id else "timeline",
            "composer_type": "share" if (not group_id and url_attachment) else "group" if group_id else "timeline",
            ...
            "audience": {
                ...
                "privacy": {
                    ...
                    "base_state": base_states[2] if allow_user_ids else base_states[base_state_idx],
                    "deny": [],
            ...
            "navigation_data": {
                "attribution_id_v2": f"CometGroupDiscussionRoot.react,comet.group,tap_search_bar,{int(time.time()*1000)},909538,2361831622," if group_id else f"ProfileCometTimelineListViewRoot.react,comet.profile.timeline.list,via_cold_start,{int(time.time()*1000)},796829,190055527696468,"
            },
        ...
        "UFI2CommentsProvider_commentsKey": "CometGroupDiscussionRootSuccessQuery" if group_id else "ProfileCometTimelineRoute",
        "hashtag": None,
